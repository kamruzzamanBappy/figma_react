      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {servicesData.map((service, index) => (
            <div
              key={index}
              className={`flex flex-col sm:flex-row md:justify-between md:items-center gap-6 p-6 cursor-pointer rounded-lg border border-[#191A23] ${service.backgroundColor} ${service.textColor}`}
            >
              <div className="md:w-1/2 flex flex-col h-full justify-between">
                ...
                <a
                  href="#"
                  className={`flex items-center gap-4 py-2 rounded-md text-black hover:bg-opcity-80 hover:text-black/80`}
                >
                  <img src={LinkIcon} alt="" className="w-6 h-6 rounded-full" />
